...
class Optimizer_SGD:
  ...
  def update_params(self, layer):
    ...
    if self.momentum:
      ...
      # current gradients
      # 设置一个介于 0 和 1 之间的参数，表示要保留的先前参数更新的分数，并
      # 从中减去（加上负数）我们的实际梯度，乘以学习率（像以前一样）
      weight_updates = (
        self.momentum * layer.weight_momentums - self.current_learning_rate * layer.dweights
      )
      # 下一次计算的动量就是这一次的更新（动量始终是参数的先前更新）
...
# Adam optimizer (Adam优化器能够有效地调整模型参数，加速训练过程并提高收敛性)
class Optimizer_Adam:
  ...
  def update_params(self, layer):
    ...
    # Get corrected cache (计算修正后的缓存以消除偏差)
    weight_cache_corrected = layer.weight_cache / (1 - self.beta_2 ** (self.iterations + 1))
    bias_cache_corrected = layer.bias_cache / (1 - self.beta_2 ** (self.iterations + 1))

    # 根据修正后的动量和缓存，使用带平方根归一化的SGD更新权重和偏置
